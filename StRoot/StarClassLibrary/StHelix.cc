...
#ifdef KEHW_DCA
        double sxy = fudgePathLength(p);
        double sz  = dz / mSinDipAngle;
        double ds  = period();
        int     n  = static_cast<int>( (sxy - sz) / ds );

        sxy -= n*ds;
        // sxy += ds * ( sxy < sz ? 1 : -1 ) * ( fabs(sxy-sz) < ds/2.0 ? 0 : 1 );
        sxy += ds * ( sxy < sz ? 1 : -1 ) * (int)( fabs(sxy-sz)/(ds/2.0) );
        s = sxy;
