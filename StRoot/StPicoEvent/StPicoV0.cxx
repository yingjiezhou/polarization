...
StPicoV0::StPicoV0(StPicoTrack *t_pos, StPicoTrack *t_neg, StMuEvent *ev, Int_t *map2Track)
{
  if(!t_pos || !t_neg || !ev) {
    ...
    if(dcaDaughters<Pico::mV0DcaDaughtersMax) {    // < 0.8cm
...
StPicoV0::StPicoV0(StPicoTrack *t_pos, StPicoTrack *t_neg, StPicoEvent *ev, Int_t *map2Track)
{
  if(!t_pos || !t_neg || !ev) {
    Clear();
  } else {
    double B = ev->bField();
    TVector3 primaryVertex = ev->primaryVertex();

    //StPhysicalHelixD helix_pos(t_pos->gMom(), t_pos->origin(), B*kilogauss, t_pos->charge());
    //StPhysicalHelixD helix_neg(t_neg->gMom(), t_neg->origin(), B*kilogauss, t_neg->charge());
    StPicoPhysicalHelix helix_pos(t_pos->gMom(), t_pos->origin(), B*kilogauss, t_pos->charge());
    StPicoPhysicalHelix helix_neg(t_neg->gMom(), t_neg->origin(), B*kilogauss, t_neg->charge());
	...
	TVector3 mom_pos = helix_pos.momentumAt(s.first, B*kilogauss);
	TVector3 mom_neg = helix_neg.momentumAt(s.second, B*kilogauss);
	TVector3 mom_v0 = mom_pos + mom_neg;
	float angle = (v0-primaryVertex).Angle(mom_v0);
	float dca2vtx = (v0-primaryVertex).Mag()*TMath::Sin(angle);
...
void StPicoV0::Clear(const Option_t* opt)
{
  ...
  mMomentum[pos].SetXYZ(0.,0.,0.); mMomentum[neg].SetXYZ(0.,0.,0.);
  mV0Pos.SetXYZ(-999.,-999.,-999.);
...
{
  if(i==pos||i==neg) return (StPicoTrack*)StPicoDst::picoArray(StPicoArrays::Track)->UncheckedAt(mIndex2Track[i]);
  //if(i==pos||i==neg) return (StPicoTrack*)StPicoDst::picoArray[StPicoArrays::Track]->UncheckedAt(i);
  //if(i==pos||i==neg) return (StPicoDst::track(i));
  else return 0;
}

TVector3 StPicoV0::momentum(const Int_t i) const
{
  TVector3 ret(0.,0.,0.);
...
void StPicoV0::setParticleHypothesis(const Int_t ip_pos, const Int_t ip_neg)
{
  ...
  TLorentzVector fourMom_pos(mMomentum[pos], (Float_t)TMath::Sqrt(mMomentum[pos].Mag2()+Pico::mMass[ip_pos]*Pico::mMass[ip_pos]));
  TLorentzVector fourMom_neg(mMomentum[neg], (Float_t)TMath::Sqrt(mMomentum[neg].Mag2()+Pico::mMass[ip_neg]*Pico::mMass[ip_neg]));
  mM = (fourMom_pos + fourMom_neg).M();
}

Float_t StPicoV0::decayLength() const
{
  StPicoEvent *ev = (StPicoEvent *)StPicoDst::picoArray(StPicoArrays::Event)->UncheckedAt(0);
  //StPicoEvent *ev = StPicoDst::event();
  return (mV0Pos - ev->primaryVertex()).Mag();
}

TVector3 StPicoV0::momentum() const
{
...
void StPicoV0::rotateTrack(const Int_t i)
{
  ...
  mMomentum[i].SetXYZ(-1.*mMomentum[i].X(), -1.*mMomentum[i].Y(), mMomentum[i].Z());
}
