...
//----------------------------------------------------------------------------------
bool StPicoCut::passV0Daughter( StPicoTrack *t, StPicoEvent *ev)
{
  if(!t) return kFALSE;
  //float dca=(t->dcaPoint()-ev->primaryVertex()).mag();
  float dca=t->gDCA(ev->primaryVertex()).Mag();
  if(t->nHitsFit()<Pico::mV0DaughterNHitsFitMin) return kFALSE;

  bool pionCand = fabs(t->nSigmaPion())<=Pico::mV0DaughterNSigmaPionMax && 
                  ( ( t->gMom().Perp()<Pico::mV0DaughterDca2VertexPtMax && dca>=Pico::mV0KsPionDca2VertexMin ) ||
                    t->gMom().Perp()>=Pico::mV0DaughterDca2VertexPtMax );
  bool protonCand = fabs(t->nSigmaProton())<=Pico::mV0DaughterNSigmaProtonMax &&
                  ( ( t->gMom().Perp()<Pico::mV0DaughterDca2VertexPtMax && dca>=Pico::mV0LambdaProtonDca2VertexMin ) ||
                    t->gMom().Perp()>=Pico::mV0DaughterDca2VertexPtMax );
...
*/
bool StPicoCut::passV0( StPicoV0 *v0, StPicoEvent *ev )
{
  if(!v0) return kFALSE;
  if(v0->dcaDaughters()>Pico::mV0DcaDaughtersMax) return kFALSE;
  TVector3 v0Mom = v0->momentum(pos) + v0->momentum(neg);
  TVector3 pVtx = ev->primaryVertex();
  if(v0Mom.Dot(v0->v0Pos()-pVtx)<=0) return kFALSE;  // V0 going away from primary vertex
...
//----------------------------------------------------------------------------------
bool StPicoCut::passKs( StPicoV0 *v0 ,StPicoEvent *ev)
{
  if(!v0) return kFALSE;
  ...
  float dca_pos=t_pos->gDCA(ev->primaryVertex()).Mag();
  float dca_neg=t_neg->gDCA(ev->primaryVertex()).Mag();
  //float dca_pos=(t_pos->dcaPoint()-ev->primaryVertex()).mag();
  //float dca_neg=(t_neg->dcaPoint()-ev->primaryVertex()).mag();
  ...
  if(t_pos->gMom().Perp()<Pico::mV0DaughterDca2VertexPtMax &&
     dca_pos<Pico::mV0KsPionDca2VertexMin) return kFALSE;
  if(t_neg->gMom().Perp()<Pico::mV0DaughterDca2VertexPtMax &&
     dca_neg<Pico::mV0KsPionDca2VertexMin) return kFALSE;
...
//----------------------------------------------------------------------------------
bool StPicoCut::passLambda( StPicoV0 *v0 , StPicoEvent *ev)
{
  if(!v0) return kFALSE;
  ...
  // daughter-pVertex dca cut for low pT tracks  -- 20200514  Yaping comment for lambda acceptance check
  /*if(t_pos->gMom().Perp()<Pico::mV0DaughterDca2VertexPtMax && 
     dca_pos<Pico::mV0LambdaProtonDca2VertexMin) return kFALSE;
  if(t_neg->gMom().Perp()<Pico::mV0DaughterDca2VertexPtMax && 
     dca_neg<Pico::mV0LambdaPionDca2VertexMin) return kFALSE;  
  */
  // typology cut
...
//----------------------------------------------------------------------------------
bool StPicoCut::passLbar( StPicoV0 *v0, StPicoEvent *ev)
{
  if(!v0) return kFALSE;
  ...
  // daughter-pVertex dca cut for low pT tracks  -- 20200514 Yaping comment for lambda acceptance check 
  /*if(t_pos->gMom().Perp()<Pico::mV0DaughterDca2VertexPtMax && 
     dca_pos<Pico::mV0LambdaPionDca2VertexMin) return kFALSE;
  if(t_neg->gMom().Perp()<Pico::mV0DaughterDca2VertexPtMax && 
     dca_neg<Pico::mV0LambdaProtonDca2VertexMin) return kFALSE;
  */
  // typology cut
  if(v0->dca2Vertex()>Pico::mV0LambdaDca2VertexMax) return kFALSE;
  if(v0->decayLength()<Pico::mV0LambdaDecayLengthMin || v0->decayLength()>Pico::mV0LambdaDecayLengthMax) return kFALSE;
