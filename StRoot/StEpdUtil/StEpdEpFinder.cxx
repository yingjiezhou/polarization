...
StEpdEpFinder::StEpdEpFinder(int nEventTypeBins, char const* OutFileName, char const* CorrectionFile) : mFormatUsed(2), mThresh(0.3), mMax(2.0), mWeightingScheme(0)
{
  ...
  if (mCorrectionInputFile->IsZombie()) {
    ...
    std::cout << "Input file: "<<CorrectionFile<<" with Correction Histograms provided" << std::endl;
    for (int ew=0; ew<2; ew++){
      for (int order=1; order<_EpOrderMax+1; order++){
  ...
  TString CorrectionOutName = OutFileName;
  CorrectionOutName += ".EpdCorrections.root";                                                                                                               
  mCorrectionOutputFile = new TFile(CorrectionOutName,"RECREATE");
  for (int ew=0; ew<2; ew++){
    for (int order=1; order<_EpOrderMax+1; order++){
  ...
  //TString ResolutionFileName = OutFileName;
  //ResolutionFileName += ".EpdResolution.root";
...
//cout<<" Inside StEpdEpFinder Prithwish: etaWeight, TileWeight, PhiWeightedTileWeight "<<etaWeight<<" "<<TileWeight<<" "<<PhiWeightedTileWeight<<endl;
    }
  }  // loop over hits
  //--------------------------------- end loop over hits ---------------------------------------
